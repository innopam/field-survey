...
export class FooService {
  ...
  async getFarmMapsByBbox(bboxQuery: BboxQueryDto): Promise<FarmMapResponseDto[]> {
    const farmMaps = await this.farmMapRepository
      .createQueryBuilder('farmMap')
      .leftJoin('farmMap.foos', 'foo', 'foo.deletedAt IS NULL')
      .addSelect('CASE WHEN COUNT(foo.id) > 0 THEN true ELSE false END', 'hasFoo')
  ...
  async upsertFooByFarmMap(farmMapId: number, fooData: CreateFooDto, uploadedFiles: string[]): Promise<FooResponseDto> {
    // 기존 FOO 데이터를 soft delete
    await this.fooRepository.softDelete({ farmMapId });

    // 팜맵 정보 조회
    const farmMap = await this.farmMapRepository.findOne({ where: { id: farmMapId } });
    if (!farmMap) {
  ...
  generateImageFileName(farmMap: FarmMap, originalName: string): string {
    ...
    const sanitizedAddress = address.replace(/[^\w\s-]/gi, '').replace(/\s+/g, '_');
      ...
      try {
        ...
        for (const photo of photos) {
          if (fs.existsSync(photo.filePath)) {
            ...
            const folderName = `${photo.foo.pnu}_${photo.foo.farmMap?.stdgAddr || 'no-address'}`
              .replace(/[^\w\s-]/gi, '')
              .replace(/\s+/g, '_');
  ...
  private mapToResponseDto(foos: Foo[]): FooResponseDto[] {
    return foos.map(foo => ({
      ...
      photos: foo.photos ? foo.photos.map(photo => ({
        id: photo.id,
        filePath: photo.filePath,
      })) : [],
      crops: foo.crops ? foo.crops.map(crop => ({
        id: crop.id,
        cropCode: crop.cropCode,
      })) : [],
    }));
